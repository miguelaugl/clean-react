import React from 'react';

import styles from './styles.scss';

export enum IconName {
  THUMB_DOWN = 'data:image/png;base64,',
  THUMB_UP = 'data:image/png;base64,',
}

type Props = {
  iconName: IconName;
  className?: string;
};

export const Icon = ({ iconName, className }: Props) => {
  const iconColor = iconName === IconName.THUMB_DOWN ? styles.red : styles.green;

  return (
    <div className={[styles.iconWrap, iconColor, className].join(' ')}>
      <img data-testid='icon' className={styles.icon} src={iconName} alt='Thumbs down' />
    </div>
  );
};
